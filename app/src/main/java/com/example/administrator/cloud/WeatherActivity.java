...
public class WeatherActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (Build.VERSION.SDK_INT >= 21){
            ...
            getWindow().setStatusBarColor(Color.TRANSPARENT);//setStatusBarColor  这个方法是21以上才可以使用，此时不使用
        }
    ...
    public void requestWeather(final String weatherId){
        /*
        * 下面的这个路径
        * http://guolin.tech/api/weather?cityid=CN101190401&key=e1984444a03d4a52a1f6cc545cce9245
        *
        * */
                ...
                final Weather weather = Utility.handleWeatherResponse(responseText);
                    ...
                    public void run() {
                        if (weather != null && "ok".equals(weather.status)){
                            ...
                            editor.apply();
                            mWeatherId = weather.basic.weatherId;//通过下拉更新数据
                            showWeatherInfo(weather);
                        }else{
                            Toast.makeText(WeatherActivity.this,"获取天气信息失败111",Toast.LENGTH_SHORT).show();
                        }
                        swipeRefresh.setRefreshing(false);//通过下拉更新数据
                    }
                });
            }
            @Override
